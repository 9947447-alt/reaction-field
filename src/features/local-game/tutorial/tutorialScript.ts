...
export const TUTORIAL_STEPS: Record<TutorialStepKey, TutorialStepInfo> = {
  ...
  PLAY_REFERENCE_CARD: {
    ...
    instruction: {
      zh: "在下方主行动操作条中，点击高亮的【普通出牌】按钮，将选中的稀 NaOH 打出作为场面基准。",
      en: "In the main action bar below, click the highlighted [Play] button to establish dilute NaOH as table reference.",
    },
  ...
  switch (stepKey) {
    ...
    case "RESPOND_WITH_CARD":
      return (
        ...
        (action.type === "PASS_RESPONSE" && action.playerId === "player_1")
      );
    default:
      return false;
  }
